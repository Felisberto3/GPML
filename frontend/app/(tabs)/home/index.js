    ...
    const fetchAllPosts = async () => {
      try {
        ...
        if (token) {
          const config = {
            headers: {
              Authorization: `Bearer ${token}` // Adiciona o token ao cabeçalho como "Bearer {token}"
            }
          };
          const response = await axios.get("http://192.168.43.222:3333/post/get/null", config);
          const userPosts = response.data;
          if (userPosts && userPosts.length > 0) {
            for (const post of userPosts) {
              const userPostId = post.usuarioId;
              const userResponse = await axios.get(`http://192.168.43.222:3333/usuario/get/${userPostId}`, config);
